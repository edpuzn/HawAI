...
async def chat_completions(
    ...
    raw_messages: List[Dict[str, Any]] = body.get("messages") or []
    # Support OpenAI-style multi-part content for images (mixed content)
            ...
            for part in content:
                ...
                if t == "text":
                    ...
                    if t == "image_url":
                        ...
                        data_url = part.get("image_base64") or part.get("image")
                    if url:
                        pending_images.append(url)
                    if data_url:
                        pending_images.append(data_url)
            messages_in.append({"role": role, "content": "\n".join(text_parts)})
        else:
            messages_in.append({"role": role, "content": content or ""})
    ...
    async with OllamaClient(model=model) as client:
        content = await client.chat_messages(messages_in, temperature=temperature, images=pending_images or None)
